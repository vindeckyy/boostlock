...
class TestConfig(unittest.TestCase):
    def test_default_config(self):
        ...
        self.assertEqual(cfg.thermal_limit_c, 85.0)
        self.assertEqual(cfg.thermal_warn_c, 75.0)
        self.assertEqual(cfg.thermal_recover_c, 70.0)
    ...
    def test_to_dict_and_from_dict(self):
        cfg = BoostLockConfig(target_frequency_khz=4200000, thermal_limit_c=80.0)
        data = cfg.to_dict()
        self.assertIsInstance(data, dict)
        self.assertEqual(data["target_frequency_khz"], 4200000)
        self.assertEqual(data["thermal_limit_c"], 80.0)

        restored = BoostLockConfig.from_dict(data)
        self.assertEqual(restored.target_frequency_khz, 4200000)
        self.assertEqual(restored.thermal_limit_c, 80.0)
        restored.validate()
